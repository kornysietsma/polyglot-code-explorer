...
- **Two cached node lists, not one** — `Viz.tsx` computes both once per `draw()`
  ...
  and "The three update paths"). `GlRenderer.setGeometry()` takes both, plus a
  `NestingStyle` and a `PatternPalette`; `setColours()` takes the fill list, a
  `fillFn`, and a `PatternPalette` (no outline list — positions are untouched on
  that path). See spec.md's "The three update paths" for the exact routing,
  including the nesting-only-edit sub-case.
...
Risks specific to steps 0–9 (Playwright GPU context, outline buffer size, quadtree
picking accuracy) are all resolved — see spec.md's "Open questions". What's left,
...
`CLAUDE.md` at step 12. Steps 0-9 are all done; their play-by-play has been
trimmed since the remaining steps (10-12) don't depend on the implementation
details - the durable facts they produced are folded into `spec.md` (module
layout, "The three update paths" including the nesting-only sub-case, "Team
pattern visualisation") or kept below only where a later step still needs them.

**Manual playwright-cli check gotchas (apply to any future manual-verify step,
...
- For a single click or hover, dispatch a real event directly rather than using
  `playwright-cli`'s synthetic mouse commands as separate CLI invocations (only
  the first one or two in a sequence reliably reach the page):
  `page.evaluate(() => el.dispatchEvent(new MouseEvent('click'/'mousemove', {clientX, clientY, bubbles: true})))`.
- For a **drag/pan gesture**, use Playwright's native input instead -
  `page.mouse.move(x,y)` then `.down()` then `.move(x2,y2,{steps})` then `.up()`
  - confirmed working (content visibly shifts). Expect a caught, harmless
    console `TypeError: Cannot read properties of null (reading 'document')` from
    d3's internal `nodrag_default` (`.chart-stack`'s `mousedowned` handler) either
    way - reproduces on any visualisation, does not reproduce on the old SVG
    renderer with the same gesture, and does not stop the pan from working. Not
    investigated further (out of scope for steps 8-9, where it was noticed) -
    worth a look during step 10's manual pan/zoom pass since it's specific to
    `d3.zoom` being attached to a plain `div` (step 1).

**Facts step 10+ needs that aren't (fully) in "Technical context" above:**
...
- To capture real WebGL buffer sizes without adding permanent instrumentation
  ...
  a similar breakdown.
- **Visualisation-switch time missed the <50 ms target as last measured (step
  8, `spring-projects.json`, 80,691 nodes):** 87 ms (Programming Language ->
  Lines of Code), 247 ms (-> Churn) - both routed correctly through
  `setColours()` alone (no geometry rebuild), but the per-node `fillFn`
  evaluation and colour fan-out (`geometry.ts`'s `buildFillAttributes`) still
  cost more than the target. Not investigated further at the time since step 8
  was about eliminating the geometry/picking-index rebuild, not this cost.
  Re-measure with the step-0 harness in step 10 and either confirm it now
  meets the target or record the shortfall explicitly (plan-level decision 4) -
  don't round it away.
